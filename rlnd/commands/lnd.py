...
def install(c, version: str = "v0.13.1-beta.rc2"):
    ...
    c.run("ln -s .lnd/logs/bitcoin/mainnet/lnd.log lnd.log")
    c.sudo("rm -rf lnd")
...
def backup_channels(c, node_name):
    ...
    c.get(
        backup_path,
        os.path.join(config_node_dir(node_name), backup_name),
    )
...
def get_graph(c):
    ...
    c.get("/tmp/graph.json", "data/graph.json")
