...
def setup(c, upgrade=True, mainnet=True):
    ...
    if c.run("test -d .rln-env.sh", warn=True, hide=True).failed:
        src_conf = (".rln-env-testnet.sh", ".rln-env.sh")[mainnet]
        c.put(get_conf_file_path(src_conf), ".rln-env.sh")
    if ".rln-env.sh" not in c.run("cat ~/.bashrc", hide=True).stdout:
        c.run("""sed -i '1isource ~/.rln-env.sh\n' ~/.bashrc""", hide=True)
